...
void Data::CheckData(char FileName[6])
{
	...
	//OpenByte Error
	Data = ReadFile(0, FileName);
	if (Data != 0x1){
		std::cout << "Error Open Byte!\n";
		OpenByteCrash = true;
	}
	//Showing Error's
		if (PositionCrash == false & ClothesCrash == false & OpenByteCrash == false)
